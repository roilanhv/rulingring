...
get_head <- function(ifile) names(fread(ifile, skip = 2, nrows =1))

fread_h <- function(ifile) {
    nm <- get_head(ifile)
    d <- fread(ifile, skip = 3)
    d <- setNames(d, nm)
    return(d)
}


sib2 <- function(exec_dir =  "~/Dropbox/Dissertacao/sib2_offline_multicamada/",
                 multicamadas = "off",
                 date1="2001010102",
                 date2="2014123124",
...
# Parâmetros de entrada para o DBHM
     paras <- paste(date1, date2, infile2, dtt, itrunk, ilw, ivtype, istype, isnow, 
         ...
         sodep,rootex,anik,satcap_c,satcap_g,
         nlayers,beta,dz,hr_proc,ksat_pre)
# Linha de comando para rodar o SiB2
    if(multicamadas == "off"){
     command_line <- paste("cd",exec_dir," ; echo",
                     "\"", 
                     paras,
                     "\"",
...
# Lendo a saída do DBHM
    ...
    if(verbose) cat("Reading sib2diag.txt","\n")
     ...
     ifile <- gsub(pattern = "'",replacement = "",
                   x = paste0(dir_out,"sib2diag",id,".txt"))
     
     if(file.exists(ifile)){
         header <- read.table(file = ifile,
                              header = TRUE,nrows = 1)
         
         fww <- as.data.frame(fread_h(ifile))
         
         # Calculando variáveis de conteúdo de água no solo [camada 0-1m, 1-2m]
         if(multicamadas == "off" & "vwc02" %in% vars_out){
             if(verbose) cat("Calculando conteudo de água nas camadas","\n")
             
             fww$swc20 <- header$poros * (fww$www1 * header$zdepth1  + (0.2 - header$zdepth1) * fww$www2) *1000
             if( (header$zdepth1[1] + header$zdepth2[1]) >= 1.0 & 
                 (header$zdepth1[1] + header$zdepth2[1]) <= 2.0){
             ...
             fww$swc01=fww$swc1
             fww$swc02=fww$swc1+fww$swc2
             # Calculando conteudo volumetrico de água no solo 
             #     
             fww$vwc20 <- ( fww$www1 * header$poros + fww$www2 * poros)/2     
             
             if( (header$zdepth1[1] + header$zdepth2[1]) >= 1.0 & 
                 (header$zdepth1[1] + header$zdepth2[1]) <= 2.0){
                 
                 fww$vwc01 <- ( fww$www1 * header$poros + fww$www2 * header$poros)/2 
                 
                 fww$vwc02 <- ( fww$www2 * header$poros + fww$www3 * header$poros )/2
                 
             } else {
                 fww$vwc01 <- (fww$www1 * header$poros + fww$www2 *  header$poros)/2
                 
                 fww$vwc02 <- fww$www2 * fww$poros
             }
