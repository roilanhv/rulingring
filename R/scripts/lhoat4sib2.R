#***************************************************************rhv*>
#    Function LHOAT do pacote hydroPSO adaptada para o modelo SiB2  >
...
function (fn = 'sib2' ,
  ...
  #-+ Checando funçao de chamada ao modelo 
    if (missing(fn)) {
      ...
      cat("Successfully loaded function",fn.name, "\n")
    } else stop("Missing argument: 'class(fn)' must be in c('function', 'character')")
      # saindo 'fn' guarda a função e 'fn.name' a chamada em character
    
  
    if (length(lower) != length(upper)) 
        stop(paste("Invalid argument: 'length(lower) != length(upper) (", 
            length(lower), "!=", length(upper), ")'", sep = ""))
    #-+ Lista com os argumentos por default para a configuração do LHOAT
  con <- list(N = 3,  # número de elementos no intervalo dos parâmetros   
              f = 0.15, # fração do intervalo dos parâmetros com o qual o valor..
              # ..inicial é determinado
              drty.in = "PSO.in", # diretorio de entrada 
              drty.out = "LH_OAT", # diretório para as saídas
              param.ranges = "ParamRanges.txt", # arquivo com os intervalos dos..
              # ..parâmetros
              digits = 7, 
              normalise = FALSE, 
              ...
              verbose = TRUE, 
              REPORT = 100, 
              parallel = c("none","multicore", "parallel", "parallelWin"), # multi-processamento
              par.nnodes = NA, 
              par.pkgs = c() 
              )
    # determinando tipo de processamento em paralelo
    parallel <- match.arg(control[["parallel"]], con[["parallel"]])
    nmsC <- names(con) # nome dos argumentos da lista 'default'
    ...
    if (fn.name == "sib2") { # Chamada a função do SiB2
        if (!file.exists(param.ranges)) # Comp se existe arquivo ParamRanges-Sens.txt
            stop(paste("Invalid argument: The file '", param.ranges, 
                "' does not exist !", sep = ""))
        if (is.null(model.FUN)) {
            stop("'model.FUN' has to be defined !")
        }
        else { # Informações da função 'sib2'
            model.FUN.name <- model.FUN # nome da função
            model.FUN <- match.fun(model.FUN) # corpo da função
        }
        if (length(model.FUN.args) == 0) {
            warning("'model.FUN.args' is an empty list. Are you sure your model doesn't have any argument(s) ?")
        }
        else { # modificando os arguments de entrada com os da chamada de LHOAT
    ...
    # Leitura do intervalo para cada parâmetro 
    if (fn.name == "sib2") {
        if (verbose) 
            message("==============================================================")
        if (verbose) 
  ...
  # -----------------------------------------------------  
    ...
    P <- nrow(X.Boundaries) # numero de parâmetros
...
# 
    ...
    if (verbose) 
        message("==============================================================")
    # Criando o primeiro set de parâmetros, através de Latin Hypercubic Sampling function
    Theta.Ini <- rLHS(n = N, ranges = X.Boundaries)
    # message(paste0(summary(Theta.Ini)[,1], collapse = "\n"), "\n",
    #         paste0(summary(Theta.Ini)[4,], collapse = "\n") )
    if (verbose) 
        message("                                                              ")
    if (verbose) 
        message("==============================================================")
...
# 
    if (fn.name != "sib2") {
        ...
        if (parallel == "none") {
            out <- llply(1:nparamsets,.progress =  "tk",
      ...
      message("===============================================================================")
      message("[ 3.1) Finalized runs. Amount of jobs executed was: ]")
      message("===============================================================================")
